...
void WiMonTerminal::gs_fetch_patient_name() {
  if (Blynk.connected()) {  // check WiFi connection
    HTTPClient http;
    http.begin(ENDPOINT_API_URL + WiMonTerminal::patientID);
    http.setFollowRedirects(HTTPC_STRICT_FOLLOW_REDIRECTS);
    http.GET();
    WiMonTerminal::patientID = http.getString();
    Serial.println("PatientName = " + WiMonTerminal::patientID);
    http.end(); 
  }
}
...
void WiMonTerminal::gs_post_vitals_record() {
  if (Blynk.connected()) {  // check WiFi connection
    if (millis() - lastlastGSPostRequest >= POST_INTERVAL) {  // Check interval
      HTTPClient http;
